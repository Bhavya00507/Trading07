// d:\Trading07\scripts\generate_verified_screenshots_v4.js
...
(async () => {
  console.log('===========================================================');
  console.log('   REAL QUANTUM TERMINAL BUYER SCREENSHOT GENERATION ENGINE V4');
  console.log('===========================================================');
  ...
  async function closeAllModals() {
    // Native click on .tv-close-btn if visible
    const tvBtn = await page.$('.tv-close-btn');
    if (tvBtn) await tvBtn.click().catch(() => {});

    // Native click on .buyer-demo-modal-close if visible
    const demoBtn = await page.$('.buyer-demo-modal-close');
    if (demoBtn) await demoBtn.click().catch(() => {});

    // Native click on .script-editor-close or modal close if visible
    const scriptBtn = await page.$('.script-editor-close');
    if (scriptBtn) await scriptBtn.click().catch(() => {});
  ...
  async function captureAndValidate(filename, expectedName, prepareStep, assertSelector) {
    ...
    // B. Assert element exists if specified
    if (assertSelector) {
      const elem = await page.$(assertSelector);
      console.log(`  Selector '${assertSelector}': ${elem ? 'FOUND' : 'NOT FOUND'}`);
    }
  ...
  await captureAndValidate('04-paper-trading.png', 'Paper Trading', async () => {
    ...
    if (!orderPanel) {
      await page.click('button[title*="Order Panel"]');
    }
  ...
  // 09 — Market Data Gateway (Market Data Gateway Stream Panel)
  ...
  // 10 — Smart Order Router (Smart Order Router / DOM Panel)
  ...
  await captureAndValidate('12-system-health.png', 'System Health', async () => {
    await page.click('button[title*="System Technical Health"]');
