// d:\Trading07\scripts\generate_verified_screenshots_v3.js
...
(async () => {
  console.log('===========================================================');
  console.log('   REAL QUANTUM TERMINAL BUYER SCREENSHOT GENERATION ENGINE V3');
  console.log('===========================================================');
  ...
  async function closeAllModals() {
    await page.evaluate(() => {
      // 1. Close Indicator Library Modal
      const tvClose = document.querySelector('.tv-close-btn');
      if (tvClose) (tvClose).click();

      // 2. Close Buyer Demo Modal / System Health Modal
      ...
      // 4. Send Escape key event to close any remaining popups
      window.dispatchEvent(new KeyboardEvent('keydown', { key: 'Escape', bubbles: true }));
    });

    // Also click Escape via Playwright
    await page.keyboard.press('Escape');
    await page.waitForTimeout(500);
  }

  // Helper validation & save function
  async function captureAndValidate(filename, expectedName, prepareStep, expectedText) {
    ...
    // B. Assert expected text/feature is present on page if provided
    if (expectedText) {
      const pageText = await page.content();
      const hasText = pageText.includes(expectedText);
      console.log(`  Expected text '${expectedText}': ${hasText ? 'CONFIRMED VISIBLE' : 'NOT FOUND'}`);
      if (!hasText) {
        console.warn(`  [WARNING] Expected text '${expectedText}' not found on page!`);
      }
    }
  ...
  }, 'Quantum Terminal — Buyer Presentation Dashboard');
