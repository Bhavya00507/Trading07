// d:\Trading07\scripts\generate_verified_screenshots_v2.js
...
(async () => {
  console.log('===========================================================');
  console.log('   REAL QUANTUM TERMINAL BUYER SCREENSHOT GENERATION ENGINE V2');
  console.log('===========================================================');
  ...
  // Helper function to close all open modals / overlays before each step
  async function closeAllModals() {
    await page.evaluate(() => {
      // 1. Close Buyer Demo Modal
      const demoClose = document.querySelector('.buyer-demo-modal-close');
      if (demoClose) demoClose.click();

      // 2. Close Indicator Library Modal
      const indClose = document.querySelector('.indicator-library-modal-close') || document.querySelector('.modal-close');
      if (indClose) indClose.click();

      // 3. Close Script Studio
      const scriptClose = document.querySelector('.script-editor-close');
      if (scriptClose) scriptClose.click();

      // 4. Close System Health Modal
      const healthClose = document.querySelector('.health-modal-close');
      if (healthClose) healthClose.click();

      // 5. Send Escape key event to close floating popups
  ...
  async function captureAndValidate(filename, expectedName, prepareStep, featureCheckSelector) {
    console.log(`\n--- Capturing ${filename} (${expectedName}) ---`);

    // Always clean up modals first
    ...
    // B. Assert expected feature selector exists in DOM if provided
    if (featureCheckSelector) {
      const elem = await page.$(featureCheckSelector);
      console.log(`Feature check '${featureCheckSelector}': ${elem ? 'FOUND' : 'NOT FOUND'}`);
    }
