// d:\Trading07\scripts\generate_verified_screenshots_v9.js
...
(async () => {
  console.log('===========================================================');
  console.log('   REAL QUANTUM TERMINAL BUYER SCREENSHOT GENERATION ENGINE V9');
  console.log('   (100% FRESH UNIQUE USER SESSION PER SCREENSHOT STEP)');
  console.log('===========================================================');

  const browser = await chromium.launch({ channel: 'chrome', headless: true });

  // Helper to register & login a FRESH user for a specific step
  async function createFreshStepSession(stepName, width = 1920, height = 1080, isMobile = false) {
    const username = `demouser_${stepName}_${Math.floor(Math.random() * 10000)}`;
    ...
    // 4. Dismiss initial BuyerDemoDashboardModal if open
    const initialClose = await page.$('.buyer-demo-modal-close');
    if (initialClose) await initialClose.click().catch(() => {});
  ...
  async function captureStep(filename, expectedName, stepKey, setupAction, width = 1920, height = 1080, isMobile = false) {
    console.log(`\n--- Capturing ${filename} (${expectedName}) ---`);

    const { context, page } = await createFreshStepSession(stepKey, width, height, isMobile);
  ...
  await captureStep('01-dashboard.png', 'Dashboard', 'dashboard', async (page) => {
    await page.evaluate(() => {
  ...
  await captureStep('02-markets.png', 'Markets', 'markets', async (page) => {
    await page.evaluate(() => {
  ...
  await captureStep('03-charting.png', 'Charting', 'charting', async (page) => {
    await page.keyboard.press('Control+i');
  });

  // 04 — Paper Trading (Institutional Order Entry Panel)
  await captureStep('04-paper-trading.png', 'Paper Trading', 'paper', async (page) => {
    await page.evaluate(() => {
  ...
  await captureStep('05-risk-portfolio.png', 'Portfolio / Risk', 'portfolio', async (page) => {
    await page.evaluate(() => {
  ...
  await captureStep('06-replay-studio.png', 'Replay Studio', 'replay', async (page) => {
    await page.evaluate(() => {
  ...
  await captureStep('07-options-desk.png', 'Options Desk', 'options', async (page) => {
    await page.evaluate(() => {
  ...
  await captureStep('08-script-studio.png', 'Script Studio', 'script', async (page) => {
    await page.evaluate(() => {
  ...
  // 09 — Market Data Gateway (Microstructure Feed Panel)
  await captureStep('09-market-data.png', 'Market Data Gateway', 'marketdata', async (page) => {
    await page.evaluate(() => {
  ...
  await captureStep('10-smart-order-router.png', 'Smart Order Router', 'sor', async (page) => {
    await page.evaluate(() => {
  ...
  await captureStep('12-system-health.png', 'System Health', 'health', async (page) => {
    await page.evaluate(() => {
  ...
  await captureStep('11-mobile-terminal.png', 'Quantum Mobile Pro', 'mobile', async (page) => {
    await page.evaluate(() => {
