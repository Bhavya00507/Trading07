// d:\Trading07\scripts\generate_verified_screenshots_v6.js
...
(async () => {
  console.log('===========================================================');
  console.log('   REAL QUANTUM TERMINAL BUYER SCREENSHOT GENERATION ENGINE V6');
  console.log('===========================================================');
  ...
  async function closeAllModals() {
    // 1. Native click on tv-close-btn if visible
    const tvBtn = await page.$('.tv-close-btn');
    if (tvBtn) await tvBtn.click().catch(() => {});

    // 2. Native click on buyer-demo-modal-close if visible
    const demoBtn = await page.$('.buyer-demo-modal-close');
    if (demoBtn) await demoBtn.click().catch(() => {});

    // 3. Native click on close button inside QUANTUM SCRIPT STUDIO dialog
    const scriptHeaderBtn = await page.$('div:has-text("QUANTUM SCRIPT STUDIO") button');
    if (scriptHeaderBtn) await scriptHeaderBtn.click().catch(() => {});

    // 4. Press Escape via Playwright native keyboard
