...
const IndicatorLibraryModal = React.lazy(() => import('./components/IndicatorLibrary/IndicatorLibraryModal').then(m => ({ default: m.IndicatorLibraryModal })));
...
const App: React.FC = () => {
  ...
  const [isIndicatorLibraryOpen, setIndicatorLibraryOpen] = useState(false);
  ...
  useEffect(() => {
    const handleGlobalKeyDown = (e: KeyboardEvent) => {
      ...
      // Ctrl+I or Cmd+I: Indicator Library Modal
      if ((e.ctrlKey || e.metaKey) && e.key.toLowerCase() === 'i') {
        e.preventDefault();
        setIndicatorLibraryOpen((prev) => !prev);
      }
    };

    const handleOpenLibrary = () => setIndicatorLibraryOpen(true);
    window.addEventListener('open-indicator-library', handleOpenLibrary);
    window.addEventListener('keydown', handleGlobalKeyDown);
    return () => {
      window.removeEventListener('open-indicator-library', handleOpenLibrary);
  ...
  return (
    <Suspense fallback={<div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100vh', background: '#070b14', color: '#8e8e93', fontSize: 12 }}>Loading Trading Terminal...</div>}>
      ...
      {/* TradingView-Style Indicator Library Modal */}
