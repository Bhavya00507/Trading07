...
function formatDataForSeriesType(formattedCandles: ChartDataPoint[], chartType: string): any[] {
  if (!formattedCandles || formattedCandles.length === 0) return [];
  if (chartType === 'heikin') {
    return convertToHeikinAshi(formattedCandles);
  }
  if (chartType === 'line' || chartType === 'area' || chartType === 'baseline') {
    return formattedCandles.map((c) => ({
      time: c.time,
      value: c.close,
    }));
  }
...
function formatSingleUpdateForSeriesType(candle: ChartDataPoint, allCandles: ChartDataPoint[], chartType: string): any {
  if (chartType === 'heikin') {
    const haList = convertToHeikinAshi(allCandles);
    return haList.length > 0 ? haList[haList.length - 1] : candle;
  }
  if (chartType === 'line' || chartType === 'area' || chartType === 'baseline') {
    return {
      time: candle.time,
...
const SingleChartCell = React.memo<SingleChartCellProps>(({
  ...
  useEffect(() => {
    ...
    console.log(`[Chart Engine] Initialized single chart cell #${cell.id} (${symbol} ${timeframe} - ${chartType}) | Primary Price Series: 1 | Market Subscriptions: 1`);
        ...
        try {
          ...
          if (olderCandles && olderCandles.length > 0) {
            ...
            if (candleSeriesInstance.current) {
              const dataToSet = formatDataForSeriesType(merged, chartType);
    ...
    const handleResize = () => {
      ...
      resizeFrameId = requestAnimationFrame(() => {
        ...
        if (mainContainerRef.current && mainChartInstance.current) {
          ...
          if (w > 0 && h > 0) {
            mainChartInstance.current.resize(w, h);
          }
        }
        if (rsiContainerRef.current && rsiChartInstance.current) {
          const w = rsiContainerRef.current.clientWidth;
          const h = rsiContainerRef.current.clientHeight;
          if (w > 0 && h > 0) rsiChartInstance.current.resize(w, h);
        }
        if (macdContainerRef.current && macdChartInstance.current) {
          const w = macdContainerRef.current.clientWidth;
          const h = macdContainerRef.current.clientHeight;
          if (w > 0 && h > 0) macdChartInstance.current.resize(w, h);
        }
        if (atrContainerRef.current && atrChartInstance.current) {
          const w = atrContainerRef.current.clientWidth;
          const h = atrContainerRef.current.clientHeight;
          if (w > 0 && h > 0) atrChartInstance.current.resize(w, h);
        }
        if (stochasticContainerRef.current && stochasticChartInstance.current) {
          const w = stochasticContainerRef.current.clientWidth;
          const h = stochasticContainerRef.current.clientHeight;
          if (w > 0 && h > 0) stochasticChartInstance.current.resize(w, h);
        }
        if (adxContainerRef.current && adxChartInstance.current) {
          const w = adxContainerRef.current.clientWidth;
          const h = adxContainerRef.current.clientHeight;
          if (w > 0 && h > 0) adxChartInstance.current.resize(w, h);
        }
        redrawCanvas();
      });
    };
    ...
    // Initial size calculation + delayed fallback for flex layout reflow
    handleResize();
    ...
    return () => {
      ...
      // Clear all series & price line refs to prevent orphaned references
      candleSeriesInstance.current = null;
      ...
      pivotPLineRef.current = null;
      pivotR1LineRef.current = null;
      pivotS1LineRef.current = null;
      pivotR2LineRef.current = null;
      pivotS2LineRef.current = null;
    };
  }, [
    ...
    const fetchAndPlot = async () => {
      try {
        ...
        if (replayActive) {
          ...
          let rawCandles = await getMarketCandles(symbol, timeframe, 1000).catch(() => []);
          if (!rawCandles || rawCandles.length === 0) {
            const globalPrice = useMarketPriceStore.getState().prices[symbol.toUpperCase()]?.currentPrice || (symbol.includes('BTC') ? 63530 : 100);
            const nowSec = Math.floor(Date.now() / 1000);
            rawCandles = Array.from({ length: 100 }).map((_, i) => {
              const t = nowSec - (100 - i) * 60;
              const base = globalPrice + Math.sin(i / 4) * (globalPrice * 0.004);
              return {
                timestamp: t * 1000,
                open: base - 2,
                high: base + 8,
        ...
        if (candleSeriesInstance.current && mainChartInstance.current) {
          const dataToSet = formatDataForSeriesType(formatted, chartType);
          candleSeriesInstance.current.setData(dataToSet as any);
          hasSetDataRef.current = true;
  ...
  useEffect(() => {
    ...
    const unsubscribe = candleEngine.subscribe(symbol, timeframe, (lastCandle) => {
      ...
      try {
        const updateToSet = formatSingleUpdateForSeriesType(formattedLast, currentData, chartType);
        candleSeriesInstance.current.update(updateToSet as any);
  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'flex', flexDirection: 'column', flex: 1, minHeight: 0, width: '100%', overflow: 'hidden', background: '#0b0d12' }}>
        {chartType === 'footprint' ? (
          <OrderFlowPanel symbol={symbol} timeframe={timeframe} />
        ) : (
          <div style={{ position: 'relative', height: numSubs === 0 ? '100%' : mainHeight, width: '100%', flex: 1, minHeight: 0 }} onContextMenu={handleContextMenu}>
            ...
            <div ref={mainContainerRef} style={{ height: '100%', width: '100%', minHeight: 0, position: 'relative' }} />
...
const Chart: React.FC<{ hideHeader?: boolean }> = ({ hideHeader = false }) => {
  ...
  return (
    <div className="chart-wrapper" style={{ display: 'flex', flexDirection: 'column', height: '100%', position: 'relative' }}>
      ...
      <IndicatorLibraryModal
        ...
        onSelectIndicator={(id) => toggleIndicator(id as any)}
      />
    </div>
  );
};
