...
import { BuyerDemoDashboardModal } from './BuyerDemo/BuyerDemoDashboardModal';
import { ProductTourModal } from './BuyerDemo/ProductTourModal';
import { AboutModal } from './BuyerDemo/AboutModal';
import { SystemHealthModal } from './BuyerDemo/SystemHealthModal';
import { DemoResetModal } from './BuyerDemo/DemoResetModal';
import './Header.css';
...
const Header: React.FC<HeaderProps> = ({
  ...
  const [isQuantumMenuOpen, setIsQuantumMenuOpen] = useState(false);
  ...
  useEffect(() => {
    const tourDone = localStorage.getItem('quantum_tour_completed');
    if (!tourDone) {
      setIsBuyerDashboardOpen(true);
    }
  }, []);

  const handleQuantumMenuTabSelect = (tabId: string) => {
    if (tabId === 'ai') {
      setAICopilotOpen(true);
    } else if (tabId === 'portfolio' || tabId === 'positions') {
  ...
  return (
    <header className="app-header">
      <div className="header-left" style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
        <button
          onClick={() => setIsQuantumMenuOpen(true)}
          style={{
          ...
          title="Open Quantum Menu"
        >
          <span style={{ fontSize: '14px', lineHeight: 1 }}>☰</span>
          <span style={{ fontSize: '10px', fontWeight: 800, color: 'var(--accent)', letterSpacing: '0.5px' }}>MENU</span>
        </button>
        <h1 className="logo">{firstWord.toUpperCase()} <span className="gold-text">{restWords.toUpperCase()}</span></h1>
        <BuyerDemoHeaderBadge onClick={() => setIsBuyerDashboardOpen(true)} />
      </div>
      ...
      <div className="header-right">
          ...
          <div style={{ display: 'flex', gap: '6px', marginRight: '16px', borderRight: '1px solid #1b2235', paddingRight: '16px' }}>
            ...
            {!isFullscreenChart && (
              <>
                ...
                <button
                  onClick={() => window.dispatchEvent(new Event('open-indicator-library'))}
                  style={{
        ...
        {/* Buyer Demo Presentation Quick Action Bar */}
        <div className="demo-presentation-controls" style={{ marginRight: '12px' }}>
          <button className="demo-action-btn" onClick={() => setIsTourOpen(true)} title="Start Guided Product Tour">
            🚀 Tour
          </button>
          <button className="demo-action-btn" onClick={() => setIsHealthOpen(true)} title="View System Technical Health">
            ⚡ Health
          </button>
          <button className="demo-action-btn" onClick={() => setIsAboutOpen(true)} title="About Quantum Terminal">
            ℹ️ About
          </button>
          <button className="demo-action-btn reset-btn" onClick={() => setIsResetOpen(true)} title="Reset Demo Environment">
            🔄 Reset
          </button>
        </div>
        ...
        <QuantumMenu
          isOpen={isQuantumMenuOpen}
          onClose={() => setIsQuantumMenuOpen(false)}
          account={account}
          onSelectTab={handleQuantumMenuTabSelect}
          onLogout={logout}
        />

        {/* Buyer Demo Modals */}
        <BuyerDemoDashboardModal
          isOpen={isBuyerDashboardOpen}
          onClose={() => setIsBuyerDashboardOpen(false)}
          onSelectModule={(key) => {
            if (key === 'ai') setAICopilotOpen(true);
            else if (key === 'script') setScriptStudioOpen(true);
            else if (key === 'mobile') window.dispatchEvent(new Event('toggle-mobile-view'));
            else if (key === 'portfolio') {
              window.dispatchEvent(new CustomEvent('navigate-tab', { detail: 'positions' }));
              window.dispatchEvent(new Event('expand-bottom-panel'));
            } else if (key === 'execution') setOrderPanelOpen(true);
            else if (key === 'dom') {
              ...
              window.dispatchEvent(new CustomEvent('navigate-tab', { detail: 'broker' }));
              window.dispatchEvent(new Event('expand-bottom-panel'));
            } else if (key === 'chart') {
              setFullscreenChart(false);
            }
        ...
        <DemoResetModal
          ...
          onConfirmReset={() => window.location.reload()}
        />
